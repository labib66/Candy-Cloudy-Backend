...
<html
    ...
    style="{{ is_object($bootstrapData->getSelectedTheme()) ? $bootstrapData->getSelectedTheme()->getCssVariables() : '' }}"
    @class(['dark' => is_object($bootstrapData->getSelectedTheme()) && $bootstrapData->getSelectedTheme('is_dark')])
>
<head>
    ...
    <meta
        name="theme-color"
        content="rgb({{ is_object($bootstrapData->getSelectedTheme()) ? $bootstrapData->getSelectedTheme()->getHtmlThemeColor() : '255, 255, 255' }})"
        data-keep="true"
    />

    @if ($fontFamily = is_object($bootstrapData->getSelectedTheme()) ? $bootstrapData->getSelectedTheme()->getFontFamily() : null)
