...
class BaseBootstrapData implements BootstrapData
{
    ...
    public function getSelectedTheme($key = null)
    {
        ...
        // Check if the themes array has any elements
        if (!empty($this->data['themes']['all'])) {
            $theme = $this->data['themes']['all'][0] ?? 'default_theme';
        } else {
            // Handle the case where there are no themes available
            $theme = null; // Or set a default theme
        }
